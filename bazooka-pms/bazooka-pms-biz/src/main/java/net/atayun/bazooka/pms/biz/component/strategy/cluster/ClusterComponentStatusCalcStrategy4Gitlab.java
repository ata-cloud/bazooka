...
@Slf4j
@Component
@StrategyNum(superClass = ClusterComponentStatusCalcStrategy.class, number = "4", describe = "集群GitLab状态计算策略")
public class ClusterComponentStatusCalcStrategy4Gitlab implements ClusterComponentStatusCalcStrategy {
    ...
    public void doCalcComponentStatus(RmsClusterConfigEntity rmsClusterConfigEntity) {
        ...
        String gitLabStatus = checkGitLabStatus(url);
        rmsClusterConfigEntity.setStatus(gitLabStatus);
     ...
     * @return
     */
    private String checkGitLabStatus(String url) {
        try {
            restTemplate.getForEntity(url, String.class);
            return ClusterStatusEnum.NORMAL.getCode();
        } catch (Exception e) {
            log.error("获取地址:[{}]对应GitLab态异常信息:[{}]", url, getFullStackTrace(e));
